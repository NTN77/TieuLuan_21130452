    ...
    const Chat = ({ targetUserId ,userNameTo }) => {
        ...
        useEffect(() => {
            axios.get(`http://localhost:8085/message/messageHistory?userIdSend=${userIdSend}&&userIdTo=${targetUserId}`,
                {
                    headers:{
        ...
        const sendMessage = () => {
            ...
            setMessages(prev => [
                ...prev,
                { userIdSend: userIdSend, userIdTo: targetUserId, content: input }
            ]);
            setInput("");
        };
                ...
                <div ref={chatRef}  style={{ height: 250, overflowY: "scroll", border: "1px solid gray", marginBottom: 10 }}>
                    {messages.map((m, i) => (
                        <div key={i} style={{ textAlign: m.userIdSend === userIdSend ? "right" : "left" }}>
                            <b>{m.userIdSend === userIdSend ? "You" : userNameTo}:</b> {m.content}
                        </div>
                    ))}
                </div>
                <input
